...
NB_MODULE(waymo_python, m) {
  ...
  el.def("get_status", &get_event_loop_status,
  ...
  el.def(
      "move_mouse",
      [](waymo_event_loop *self, unsigned int x, unsigned int y,
         bool relative) { move_mouse(self, x, y, relative); },
      nb::arg("x"), nb::arg("y"), nb::arg("relative"),
      "Moves the mouse around the screen");

  el.def(
      "click_mouse",
      [](waymo_event_loop *self, MBTNS btn, unsigned int clicks,
         uint32_t hold_ms) { click_mouse(self, btn, clicks, hold_ms); },
  ...
  el.def(
      "press_key",
      [](waymo_event_loop *self, char key, std::optional<uint32_t> interval_ms,
         bool down) {
  ...
  el.def(
      "type",
      [](waymo_event_loop *self, const char *text,

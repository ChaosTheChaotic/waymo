...
public:
    static Napi::Object Init(Napi::Env env, Napi::Object exports) {
        Napi::Function func = DefineClass(env, "WaymoLoop", {
    ...
    WaymoLoop(const Napi::CallbackInfo& info) : Napi::ObjectWrap<WaymoLoop>(info) {
        this->loop = create_event_loop(NULL);
    }

    ~WaymoLoop() {
        if (this->loop) destroy_event_loop(this->loop);
    }
